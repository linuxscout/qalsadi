...
class qalsadiTagCoderTestCase(unittest.TestCase):
    ...
    def _check_word_tags(self, word, vocalized_lemma=False, check_as=""):
        ...
        stmnode = self._check_word(word, vocalized_lemma=vocalized_lemma, check_as=check_as)
    ...
    def _check_word_tags_tuple(self, word):
        ...
        for wd in wordcases:
            ...
            tuple_list.append({"vocalized":vocalized,
            "lemma": lemma,
            "tags": tags,
            "tagscode": tagscode,
            "inflect": inflect,
    ...
    def _test_many_analysis_tags(self, words_lemmas, test_field = "tagscode", limit=10):
        ...
        for token in words_lemmas:
            ...
            if test_field in ("tagscode", "inflect","tags"):
                ...
                expected_fields = [d.get(test_field,"") for d in expected_tuple_list]
            else:
                fields = [d.get("tagscode","") for d in tuple_list]
                expected_fields = [d.get("tagscode","") for d in expected_tuple_list]                
    ...
    def test_analysis_tags_case1(self, ):
        """ test case according to generated tagcodes
        based on dataset"""
       
        result  = self._test_many_analysis_tags(self.word_lemma_list, test_field="tagscode", limit=self.limit)
    ...
    def test_analysis_tags_case2(self, ):
        """ test case according to generated inflections
        based on dataset"""
       
        result  = self._test_many_analysis_tags(self.word_lemma_list, test_field="inflect", limit=self.limit)
        len_wrong_cases = len(result)
        if(len_wrong_cases):
            print("Wrong cases")
            pprint.pprint(result)
    ...
    def test_analysis_tags_case3(self, ):
        """ test case according to generated tags
        based on dataset"""
       
        result  = self._test_many_analysis_tags(self.word_lemma_list, test_field="tags", limit=self.limit)
        len_wrong_cases = len(result)
        if(len_wrong_cases):
            print("Wrong cases")
            pprint.pprint(result)
