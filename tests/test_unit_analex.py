...
class qalsadiAnalyzerTestCase(unittest.TestCase):
    ...
    def test_text_cases(self, ):
        ...
        lemmas = self.analyzer.lemmatize_text(text)
    ...
    def test_generate_data_set(self, ):
        ...
        expected_lemmas = ['هل', 'احتاج', 'إلى', 'ترجمة', 'كي', 'تف', 'خطاب', 'ملك', '؟', 'لغة', '"', 'كلاسيكي', '"(', 'فصحى', ')', 'موجود', 'في', 'كل', 'لغة', 'كذلك', 'لغة', '"', 'دارج', '"..', 'فرنسة', 'التي', 'درس', 'في', 'مدرس', 'ليست', 'فرنسة', 'التي', 'استخدم', 'ناس', 'في', 'شوارع', 'باريس', '..', 'ملك', 'بريطاني', 'لا', 'خطب', 'بلغة', 'شوارع', 'أدان', '..', 'كل', 'مقام', 'مقال']
        self.analyzer.set_vocalized_lemma()
        lemmas = self.analyzer.lemmatize_text(text,  return_pos=True)
    ...
    def _test_many_lemmatization(self, words_lemmas, vocalized = False, limit=10):
        ...
        for item in words_lemmas:
            ...
            lemma, wordtype = self.analyzer.lemmatize(token, return_pos=True)
            ...
            if lemma != expected_lemma:
                if result_flag:
                    nb_diff += 1
                    wrong_lemmatization.append({"token": token,
                    "output": lemma,
    ...
    def test_word_cases(self, ):
        ...
        # ~ word = "يحتاج"
        # ~ lemma = 'احتاج'
        # ~ self.assertEqual(self.analyzer.check_word(word), lemma)
        # ~ word = "وفي"
        # ~ lemma = 'في'
        # ~ self.assertEqual(self.analyzer.check_word(word), lemma)
        word = "يحتاج"
        lemma = 'اِحْتَاجَ'
        results = StemNode(self.analyzer.check_word(word))
        result = results.get_lemmas()[0]
        self.assertEqual(result, lemma)


    @unittest.skip("not yet ready ")
    def test_word_cases_tuple(self, ):
        """ test word case"""
        word = '"'
        result = self.analyzer.check_word(word, return_pos = True)
        self.assertIsInstance(result, tuple, " Result Type is %s"%type(result))

    # cases

    @unittest.skip("not yet ready ")  
    def test_lemmatization_case1(self, ):
        """ test case
        based on dataset"""
