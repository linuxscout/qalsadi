...
class NounStemmer:
        ...
        # stem reduced noun : level two
                for stem in list_stem:
                    detailed_result.extend(
                        self.steming_second_level(noun, stem, procletic_nm,
                                                  encletic_nm))
    ...
    def is_compatible_proaffix_affix(self, noun_tuple, procletic_nm, encletic,
                                     suffix):
        """
        Verify if proaffixes (sytaxic affixes) are compatable
        with affixes ( conjugation)
        @param procletic_nm: first level prefix.
        @type procletic_nm: unicode.
        ...
        procletic_tags = snconst.COMP_PREFIX_LIST_TAGS[procletic_nm]['tags']
...
def get_enclitic_variant(enclitic_voc, suffix_voc):
    ...
    if enclitic_voc.startswith(ar.HEH + ar.DAMMA) and suffix_voc.endswith(
            ar.KASRA):
        encl_vo_no_inflect_mark = enclitic_voc.replace(
            ar.HEH + ar.DAMMA, ar.HEH)
...
def get_word_variant(word, suffix, encletic):
    ...
    encletic_nm = ar.strip_tashkeel(encletic)
...
def vocalize(noun, proclitic, suffix, enclitic):
    ...
    word_non_irab_mark = re.sub(u"(%s)+" % ar.FATHA, ar.FATHA,
                                word_non_irab_mark)
    word_non_irab_mark = re.sub(
        u"(%s%s%s)+" % (ar.FATHA, ar.ALEF_MAKSURA, ar.KASRATAN),
        ar.FATHATAN + ar.ALEF_MAKSURA, word_non_irab_mark)
    word_non_irab_mark = re.sub(u"%s%s%s" % (ar.FATHA, ar.ALEF_MAKSURA,
                                             ar.KASRA),
                                ar.FATHA + ar.ALEF_MAKSURA, word_non_irab_mark)
    word_non_irab_mark = re.sub(u"%s[%s|%s|%s]" % (ar.ALEF_MAKSURA, ar.DAMMA,
                                                   ar.FATHA, ar.KASRA),
    ...
    word_vocalized = re.sub(u"(%s)+" % ar.FATHA, ar.FATHA, word_vocalized)
    word_vocalized = re.sub(u"%s%s%s" % (ar.FATHA, ar.ALEF_MAKSURA,
                                         ar.KASRATAN),
    ...
    word_vocalized = re.sub(u"%s%s%s" % (ar.FATHA, ar.ALEF_MAKSURA,
                                         ar.FATHATAN),
                            ar.FATHATAN + ar.ALEF_MAKSURA, word_vocalized)
    word_vocalized = re.sub(u"%s%s%s" % (ar.FATHA, ar.ALEF_MAKSURA, ar.KASRA),
                            ar.FATHA + ar.ALEF_MAKSURA, word_vocalized)
    word_vocalized = re.sub(u"%s[%s|%s|%s]" % (ar.ALEF_MAKSURA, ar.DAMMA,
                                               ar.FATHA, ar.KASRA),
...
def validate_tags(noun_tuple, affix_tags, procletic_nm, encletic_nm,
                  suffix_nm):
    """
    ...
    if encletic_nm in (u"هم", u"هن", u"كما", u"كم",
