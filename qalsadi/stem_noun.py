...
class NounStemmer:
    ...
    def stemming_noun(self, noun_in):
        ...
        for noun in noun_list:
            ...
            list_seg_comp = self.verify_affix(
                noun, list_seg_comp, SNC.COMP_NOUN_AFFIXES
            )
    ...
    def __check_clitic_affix(self, noun_tuple, proclitic_nm, enclitic, suffix):
        ...
        affix = "-".join(
            [proclitic_nm, enclitic, suffix, str(bool(noun_tuple["mamnou3_sarf"]))]
        )
    ...
    def vocalize(self, noun, proclitic, suffix, enclitic):
        ...
        if not key in self.noun_vocalize_cache:
            self.noun_vocalize_cache[key] = self.generator.vocalize(
                noun, proclitic, suffix, enclitic
            )
    ...
    def get_noun_variants(noun):
        ...
        if ar.ALEF_MADDA in noun:
            ...
            noun_list.append(noun.replace(ar.ALEF_MADDA, ar.ALEF_HAMZA_ABOVE + ar.ALEF))
        return noun_list
    ...
    def validate_tags(noun_tuple, affix_tags, proclitic_nm, enclitic_nm, suffix_nm):
        ...
        if (
            enclitic_nm in ("هم", "هن", "كما", "كم", "هما")
            and not noun_tuple["hm_suffix"]
        ):
            return False
