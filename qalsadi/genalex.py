...
def generate_noun_forms(word):
    ...
    vocalizer = noun_affixer.muwaled()
    noun_forms = []
    # ~ word = u"قَصْدٌ"
    for element in itertools.product(procletics, suffixes, enclitics):
        ...
        suff = element[1]
        enc = element[2]
        if "-".join([proc, enc]) in SNC.COMP_NOUN_AFFIXES:
            if noun_affixer.check_clitic_affix(proc, enc, suff):
                # ~ print(arepr(element))
...
def generate_verb_forms(word):
    ...
    vocalizer = verb_affixer.muwaled()
    # ~ word = u"قصد"
    verb_forms = []
    for element in itertools.product(procletics, prefixes, suffixes, enclitics):
        ...
        suff = element[2]
        enc = element[3]
        if "-".join([pref, suff]) in SVC.VERBAL_CONJUGATION_AFFIX:
            if verb_affixer.check_clitic_affix(proc, enc, pref + "-" + suff):
                # ~ print(arepr(element))
                conj_verb = "".join([pref, word, suff])
